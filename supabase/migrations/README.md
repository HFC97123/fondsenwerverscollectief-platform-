...
in de deadline-view (bron + tier i.p.v. uitsluitend tier). Bestand 22 legt de
nieuwe accountarchitectuur vast: directe toegang na registratie (geen
handmatige goedkeuring meer), een aparte onboardingtabel, losse
productaankopen en een veilige RPC om een proefperiode te starten.
...
| 22 | `20260901194247_account_directe_toegang_onboarding_aankopen_trial.sql` | Accountarchitectuur: `handle_new_user()` zet nieuwe profielen voortaan direct op `status = 'approved'` (geen handmatige goedkeuring meer) + backfill van de ene bestaande `pending`-rij; nieuwe tabellen `profile_onboarding` (onboardingantwoorden, nooit een toegangscriterium) en `product_aankopen` (losse eenmalige aankopen, technisch los van het abonnement) met eigen RLS + GRANT; nieuwe RPC's `admin_grant_purchase` (admin-only) en `start_trial` (server-side vastgelegde duur: 7 dagen Pro / 24 uur Premium, eenmalig, nooit voor admins of actieve betalers). |
...
Migratie 22 is de eerste migratie van de accountarchitectuur-opdracht: geen
handmatige goedkeuring meer, aparte onboarding-/aankooptabellen, en de
start_trial-RPC.
...
live database reproduceert; dat is alleen waar voor de 22 bestanden
hierboven. Zie de README in die map voor de status van het voorstel zelf.

## Reproduceerbaarheid

Een nieuwe, lege Supabase-/Postgres-omgeving die uitsluitend de 22 bestanden
